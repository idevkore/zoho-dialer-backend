...
**Verified example** (US `accounts.zoho.com`, HaulOS dialer backend on `haulos-zoho-dialer-backend.atiny.cloud`, scope `ZohoCRM.modules.ALL` — `client_id` must match your Zoho app):
...
After sign-in, Zoho sends you to **`.../zoho/oauth/callback?code=...`** (and may add other Zoho params). **Append `&tenantId=kore`** (or `?tenantId=kore` if there is no existing query string) so the backend knows which tenant’s `ZOHO_*` client secrets to use, then load that full URL. See [Zoho Accounts OAuth — authorization request](https://www.zoho.com/crm/developer/docs/api/v2.1/auth-request.html) for parameters and regional hosts.
