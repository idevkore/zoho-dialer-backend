...
In **production**, set `PUBLIC_BASE_URL` to the same origin Twilio uses (required for `X-Twilio-Signature` validation). In **non-production**, validation runs by default as well; set `SKIP_TWILIO_SIG_VALIDATION` to `true`, `1`, or `yes` (trimmed) to bypass signature checks while iterating on TwiML, or when Twilio hits a tunnel URL that does not match `PUBLIC_BASE_URL`. **If the running process has `NODE_ENV=production` (for example PM2 `env_production`), validation always runs and this flag is ignored.** The Postman collection folder **📞 Twilio Webhooks** includes a pre-request script that recomputes `X-Twilio-Signature` from the resolved request URL and sorted form fields when you set the secret collection variable `twilioAuthToken` to your tenant’s Twilio Auth Token (clear it or use a real token—not the placeholder text `Enter value`, which would produce a bogus signature).
